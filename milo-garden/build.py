...
bank=json.loads((ROOT/'gardens.json').read_text());bank['puzzles']+=json.loads((ROOT/'challenges.json').read_text())['puzzles']
html=html.replace('__FIREFLY_DATA__',json.dumps(bank,separators=(',',':'))).replace('__GARDEN_CLASS__','wide').replace('__STORE__','milo:garden:results:v1')
a=html.index('  <nav class="garden-modes"');b=html.index('</nav>',a)+len('</nav>')
html=html[:a]+'''  <div class="difficulty-choice" aria-label="Puzzle difficulty"><button id="gentleGardens" aria-pressed="true">Evening walks · 365</button><button id="hardGardens" aria-pressed="false">Deep woods · 24 challenging</button></div><p id="difficultyDescription" class="difficulty-description"></p><nav class="garden-library" aria-label="Choose an evening walk"><label for="gardenSelect">Your evening walks<select id="gardenSelect"></select></label><span id="collectionProgress"></span></nav>
